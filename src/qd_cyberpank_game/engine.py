...
def calculate_investments(investments_df: pd.DataFrame, markets_df: pd.DataFrame, fund_speed: float):
    approved_invest_bids = investments_df.loc[investments_df['status'] == 1, :]
    approved_invest_bids['duration'] = approved_invest_bids['timestamp_approved'] - approved_invest_bids['timestamp_created']
    approved_invest_bids['funded_amount'] = fund_speed * approved_invest_bids['duration'].dt.seconds
    approved_invest_bids['funded_amount'] = approved_invest_bids[['funded_amount', 'amount']].min(axis=1)
...
def calculate_market(x: np.ndarray, capacity: float, min_capacity: float) -> Tuple[np.ndarray, float]:
    ...
    jackpot = x.sum() / 2
    new_capacity = np.maximum(capacity + jackpot - y.max(), min_capacity)
    return incomes, new_capacity


def generate_stocks(profits: np.array) -> pd.DataFrame:
    pass
